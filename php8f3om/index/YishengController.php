...
class YishengController extends CommonController {
    ...
    public function login(){
        ...
        $password = isset($_REQUEST['password'])?$_REQUEST['password']:"";
                        $sql = "select * from `yisheng` where `yishenggonghao` = '".$username."' and `mima` = '".$password."'";
    ...
    public function register(){
        ...
        $postData = json_decode($tmpData,true);
                        ...
                        $colum = "yishenggonghao";
        $trues = "select * from `yisheng` where `yishenggonghao` = '".$postData[$colum]."'";
    ...
    public function resetPass(){
        $username = $_REQUEST['username'];
                        $counts = "select * from `yisheng` where yishenggonghao = '".$username."'";
        $cotte = table_sql($counts);
        if($cotte->num_rows<1){
            exit(json_encode(['code'=>500,'msg'=>"用户名错误。"]));
        }
        $sql = "update yisheng set mima = '123456' where yishenggonghao = '".$username."'";
                                                                                                        		        $result = table_sql($sql);
        ...
        if ($base['isAdmin']!=1 || ($base['isAdmin']==1 && $base['tablename'] != "users")){
            ...
            if (isset($_SESSION[$md5]) && in_array($colum, $columData)){
                if($base['tablename'] == '${column.authTable}'){
                    $where .= " and `yishenggonghao` = '".$_SESSION[$md5]."'";
                }
            }
        }
